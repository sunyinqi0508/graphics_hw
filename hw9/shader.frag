...
uniform vec3 V0;
uniform sampler2D uSampler[ns + 5]; //ns + vs
varying vec3 norm;
varying float id;
varying vec3 glpos;
varying vec3 texPos;
...
void main(){
   ...
   if(id < -5.5){gl_FragColor = vec4(texture2D(uSampler[ns + 4], (1.+texPos.xy*2.5)/2.).xyz, 1.); return;} //pjsk
   else if(id < -4.5){gl_FragColor = vec4(texture2D(uSampler[ns + 3], (1.+texPos.xy*2.5)/2.).xyz, 1.); return;} //pjsk
   else if(id < -2.5){gl_FragColor = vec4(texture2D(uSampler[ns + 2], (1.+texPos.xy*2.5)/2.).xyz, 1.); return;} //pjsk
